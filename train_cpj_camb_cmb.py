...
class CosmoPowerJAXTrainer:
    ...
    def train(self, params: np.ndarray, spectra: np.ndarray,
              ...
              validation_spectra: Optional[np.ndarray] = None) -> Tuple[List[Tuple], List[Tuple]]:
        ...
        # JIT compile loss and update functions
...
def main():
    ...
    # Configuration
    probe = 'cmb_tt'  # Can be 'cmb_tt', 'cmb_ee', 'cmb_te', 'cmb_pp', 'mpk_lin', 'mpk_boost', etc.
    model_filename = 'trained_cp_jax_model.pkl'
    n_samples = 42614  # Number of training samples
    n_test = 9531      # Number of test samples
    ...
    config = TrainingConfig(
        ...
        learning_rate=1e-2,
        batch_size=512,
        n_epochs=10000,           # Fewer epochs for demonstration
    ...
    training_parameters = {key: training_parameters[key] for key in training_parameters.keys()}
    training_features = np.load('./camb_tt_training_log_spectra.npz')

    params = np.column_stack([training_parameters[key] for key in training_parameters])
    spectra = 10.**training_features['features']
    parameters = list(training_parameters.keys())
    modes = training_features['modes']

    print(f"Parameter shape: {params.shape}")
    print(f"Spectra shape: {spectra.shape}")
    print(f"Parameters: {parameters}")
    print(f"Number of multipoles: {modes.shape}")

    # Train the model
    print("Training the model...")
    weights, hyper_params = trainer.train(params, spectra)
    ...
    try:
        ...
        test_params = np.load('./camb_tt_testing_params.npz')
        test_params = {key: test_params[key] for key in test_params.keys()}
        test_params = np.column_stack([test_params[key] for key in test_params])
        # test_spectra = 10.**np.load('./camb_cmb_tt_training_data/camb_tt_testing_log_spectra.npz')['features']
    ...
    test_params = {key: test_params[key] for key in test_params.keys()}
    test_params = np.column_stack([test_params[key] for key in test_params])
    test_spectra_true = 10.**np.load('./camb_tt_testing_log_spectra.npz')['features']
