...
# Load training data from the provided .npz file
...
class CosmoPowerJAXTrainer:
    ...
    def plot_training_history(self, epochs: List[int], train_losses: List[float], val_losses: List[float], learning_rates: List[float], current_time: str, run_name: str):
        ...
        plt.subplot(1, 3, 3)
        plt.semilogy(epochs, learning_rates, 'g-', label='Learning Rate', linewidth=2)
        plt.xlabel('Epoch')
...
def main():
    ...
    n_samples = 9000  # Number of training samples
    n_test = 1000      # Number of test samples
    
    # Generate timestamp and run name for file naming
    current_time = datetime.now().strftime("%y%m%d_%H%M%S")
    run_name = f"{probe}_training_data_run"  # You can customize this run name as needed
    ...
    config = TrainingConfig(
        ...
        learning_rate=1e-2,
        batch_size=512,
        n_epochs=100,           # Fewer epochs for demonstration
    ...
    try:
        ...
        test_params, _, _, _ = training_data['cosmo_params_samples'][9000:], training_data['power_spectra'][9000:], training_data['cosmo_params_names'], training_data['k_modes']
    ...
    # Generate test parameters
    test_params, test_spectra_true, _, _ = training_data['cosmo_params_samples'][9000:], training_data['power_spectra'][9000:], training_data['cosmo_params_names'], training_data['k_modes']
